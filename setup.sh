...
# Check for llama-server or llama-cli (llama.cpp binaries)
if command -v llama-server &>/dev/null; then
    LLAMACPP_AVAILABLE=true
    LLAMACPP_BIN="llama-server"
    info "Detected: llama-server ($(command -v llama-server))"
elif command -v llama-cli &>/dev/null; then
    LLAMACPP_AVAILABLE=true
    LLAMACPP_BIN="llama-cli"
    info "Detected: llama-cli ($(command -v llama-cli))"
elif command -v llama.cpp &>/dev/null; then
    LLAMACPP_AVAILABLE=true
    LLAMACPP_BIN="llama.cpp"
    info "Detected: llama.cpp ($(command -v llama.cpp))"
fi

echo ""
echo -e "${BOLD}Select your LLM backend:${RESET}"
echo ""
echo -e "  1) LM Studio  — local server, port 1234, OpenAI-compatible${LMSTUDIO_AVAILABLE:+  ${GREEN}(detected)${RESET}}"
echo -e "  2) llama.cpp  — llama-server, port 8080, OpenAI-compatible${LLAMACPP_AVAILABLE:+  ${GREEN}(detected)${RESET}}"
echo -e "  3) Ollama     — local server, port 11434${OLLAMA_AVAILABLE:+  ${GREEN}(detected)${RESET}}"
echo    "  4) OpenAI     — cloud API (requires OPENAI_API_KEY)"
echo    "  5) Skip       — I will edit .env manually"
echo ""
...
EOF
    ...
    2)  # llama.cpp
        info "Configuring for llama.cpp (llama-server)..."
        read -rp "  Port llama-server will listen on [default: 8080]: " LC_PORT
        LC_PORT="${LC_PORT:-8080}"
        read -rp "  Path to your GGUF model file [optional, for reference]: " LC_MODEL_PATH

        cat > .env << EOF
# MAMGA-Local — generated by setup.sh on $(date +%Y-%m-%d)
LLM_BACKEND=llamacpp
LLM_MODEL=local-model
LLM_BASE_URL=http://localhost:${LC_PORT}/v1
...
LOG_LEVEL=INFO
EOF
        # Append model path as a comment if provided
        if [ -n "$LC_MODEL_PATH" ]; then
            echo "" >> .env
            echo "# GGUF model used with llama-server:" >> .env
            echo "# LLAMA_CPP_MODEL_PATH=${LC_MODEL_PATH}" >> .env
        fi

        success ".env written for llama.cpp (port ${LC_PORT})"
        echo ""
        echo -e "${YELLOW}  ▸ Start the server before running MAMGA-Local:${RESET}"
        if [ -n "$LC_MODEL_PATH" ]; then
            echo -e "${CYAN}    llama-server -m ${LC_MODEL_PATH} --port ${LC_PORT} --host 127.0.0.1${RESET}"
        else
            echo -e "${CYAN}    llama-server -m /path/to/model.gguf --port ${LC_PORT} --host 127.0.0.1${RESET}"
        fi
        echo -e "${YELLOW}  ▸ Install llama.cpp if missing: https://github.com/ggerganov/llama.cpp${RESET}"
        ;;
